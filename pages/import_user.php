...
include('function/function.php'); 
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (empty($errors)) {
			...
			$query_in="INSERT INTO tbupload_user(time,name_file,link_download) VALUES ('{$time}','{$name_file}','{$link}')";
			...
			if ($data) {
				...
				foreach ($data as $row ) {
					if ($count>1) {
	    				...
	    				$query_sl="SELECT user FROM tbuser WHERE user='{$a1}'";
	    				$result_sl=mysqli_query($dbc,$query_sl);
	    				check_query($result_sl,$query_sl);
	    				if (mysqli_num_rows($result_sl) == 1) {
							$count_errors++;
						}else{
							$query_in = "INSERT INTO tbuser(user,password,email,name,avatar,st,online,level) VALUES ('{$a1}','{$password}','{$a2}','{$a3}','{$avatar}',2,0,1)";
							$result_in = mysqli_query($dbc,$query_in);
							check_query($result_in,$query_in);
							$count_sucessful++;
						}
 ...
 ?>
    <div id="page-wrapper">
        <div class="row">
        	<div class="col-lg-12">
        		<h1 class="page-header"><li class="fa fa-download"></li><strong> Import User </strong></h1>
        	</div>
        </div>
        <div class="row">
        	<div class="col-lg-12">
        		<form name="import_user" method="POST" enctype="multipart/form-data"">
			    	<div class="panel panel-default">
			    		<div class="panel-heading">
			    			<button type="submit" name="submit" class="btn btn-success"><li class="fa fa-download"></li> <strong>Import</strong></button>
			    			<a href="delete_upload_all_user.php" class="btn btn-warning" onclick="return confirm('Are you sure clear upload history ?')"><li class="fa fa-eraser"></li> <strong>Clear History</strong></a>
									        ...
									        <?php
									        	if (isset($count_sucessful)) {
									          	?>
													  ...
													  <li class="fa fa-check"></li><strong> Yes!</strong> There were <?php echo $count_sucessful; ?> user added to the list user.
													</div>
												<?php
												}
									          	...
									          	?>
													  ...
													  <li class="fa fa-exclamation-triangle"></li><strong> Oh snap!</strong> There are <?php echo $count_errors; ?> duplicate user.
													</div>
												<?php
												}
											?>
							    		...
							    		<div class="panel-body">
											<div class="form-group">
												<table class="table table-striped table-bordered table-hover">
					            					...
					            					<tbody>
					            					<?php
					            						$query_sl="SELECT id,time,name_file,link_download FROM tbupload_user ORDER BY id DESC";
					            						$result_sl=mysqli_query($dbc,$query_sl);
			        									...
			        									?>
						            						<tr>
							            						...
							            						<td><a href="delete_upload_user.php?id=<?php echo $upload['id']; ?>" onclick="return confirm('Are you sure delete ?')"><li class="fa fa-trash-o fa-2x"></li></a></td>
						            						</tr>
						            					<?php
						            					$ord++;
						            					}
